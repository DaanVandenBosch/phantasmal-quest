...
import { QuestNpc, QuestObject, Section, NpcType, ObjectType } from '../domain';

test('create geometry for quest objects', () => {
    const object = new QuestObject(7, 13, { x: 17, y: 19, z: 23 }, ObjectType.PrincipalWarp);
    const sect_rot = 0.6;
    const sect_rot_sin = Math.sin(sect_rot);
    const sect_rot_cos = Math.cos(sect_rot);
    const geometry = create_object_geometry(object, [new Section(13, [29, 31, 37], sect_rot)]);
...
test('create geometry for quest NPCs', () => {
    ...
    const sect_rot_sin = Math.sin(sect_rot);
    const sect_rot_cos = Math.cos(sect_rot);
    const geometry = create_npc_geometry(npc, [new Section(13, [29, 31, 37], sect_rot)]);
...
test('geometry position changes when entity position changes element-wise', () => {
    const npc = new QuestNpc(7, 13, { x: 17, y: 19, z: 23 }, NpcType.Booma);
    const geometry = create_npc_geometry(npc, [new Section(13, [0, 0, 0], 0)]);
    npc.position.x += 2;
    npc.position.y += 3;
    npc.position.z += 5;
